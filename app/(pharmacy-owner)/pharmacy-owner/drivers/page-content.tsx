...
const PharmacyDriversList = dynamic(
  () =>
    import("@/features/pharmacy-owner/components/pharmacy-drivers-list").then(
      (mod) => ({
        default: mod.PharmacyDriversList,
      }),
    ),
  {
    loading: () => (
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">
              {t("pharmacyOwner.drivers.title")}
            </h1>
            <p className="text-muted-foreground">
              {t("pharmacyOwner.drivers.subtitle")}
            </p>
          </div>
        </div>
      </div>
