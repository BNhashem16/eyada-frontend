...
import { User, Mail, Phone, MessageSquare, AlertCircle, Lightbulb } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => setValue('type', 'COMPLAINT', { shouldValidate: true })}
            className={cn(
          ...
          >
            ...
            />
            <span className="text-sm font-medium">{t('feedback.complaint')}</span>
          </button>

          <button
            type="button"
            onClick={() => setValue('type', 'SUGGESTION', { shouldValidate: true })}
            className={cn(
              'flex flex-col items-center gap-2 rounded-xl border-2 p-4 transition-all',
