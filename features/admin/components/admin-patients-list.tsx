...
const getStatusConfig = (t: (key: string) => string): Record<PatientStatus, { label: string; variant: 'warning' | 'success' | 'error' | 'secondary'; icon: typeof Clock }> => ({
  [PatientStatus.PENDING]: { label: t('admin.patients.underReview'), variant: 'warning', icon: Clock },
  [PatientStatus.APPROVED]: { label: t('admin.patients.approved'), variant: 'success', icon: UserCheck },
  [PatientStatus.REJECTED]: { label: t('admin.patients.rejected'), variant: 'error', icon: XCircle },
  [PatientStatus.SUSPENDED]: { label: t('admin.patients.suspended'), variant: 'error', icon: Ban },
});
...
export function AdminPatientsList() {
  ...
  const isPending = approvePatient.isPending || rejectPatient.isPending || suspendPatient.isPending;
      ...
      <Card className="mb-6">
        <CardContent className="p-4">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <SearchableSelect
              options={[
                { value: '', label: t('admin.patients.allStatuses'), icon: <Clock className="h-4 w-4" /> },
                { value: PatientStatus.PENDING, label: t('admin.patients.underReview'), icon: <Clock className="h-4 w-4 text-warning-500" /> },
                { value: PatientStatus.APPROVED, label: t('admin.patients.approved'), icon: <UserCheck className="h-4 w-4 text-success-500" /> },
                { value: PatientStatus.REJECTED, label: t('admin.patients.rejected'), icon: <XCircle className="h-4 w-4 text-error-500" /> },
                { value: PatientStatus.SUSPENDED, label: t('admin.patients.suspended'), icon: <Ban className="h-4 w-4 text-error-500" /> },
              ]}
      ...
      {meta && (
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-muted-foreground">
            {t('admin.patients.totalResults')} <span className="font-semibold">{meta.total}</span>
          </p>
          <p className="text-sm text-muted-foreground">
            {t('admin.patients.pageOf').replace('{current}', String(meta.page)).replace('{total}', String(meta.totalPages))}
          </p>
        </div>
      )}
      ...
      {patients.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">{t('admin.patients.noPatientsFound')}</h3>
            <p className="text-muted-foreground">{t('admin.patients.noPatientsMatchFilters')}</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
                ...
                <CardContent className="p-6">
                  <div className="flex flex-col md:flex-row md:items-start gap-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {(patient.status === PatientStatus.REJECTED || patient.status === PatientStatus.SUSPENDED) && (
                        <Button
                          size="sm"
                          onClick={() => {
                            setSelectedPatient(patient.id);
