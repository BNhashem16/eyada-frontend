...
export function AdminAppointmentsList() {
  ...
  const { data: appointmentsResponse, isLoading, isError } = useAdminAppointments(filters);
  ...
  const handleUpdateStatus = (appointment: AdminAppointment, newStatus: AppointmentStatus) => {
    updateAppointment.mutate({
      id: appointment.id,
      status: newStatus,
    });
  ...
  const handleUpdatePaymentStatus = (appointment: AdminAppointment, newPaymentStatus: PaymentStatus) => {
    updateAppointment.mutate({
      id: appointment.id,
  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-4 mb-6">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('admin.appointments.total')}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('admin.appointments.completed')}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('admin.appointments.pending')}</p>
                <p className="text-2xl font-bold text-warning-600">{statistics?.pending || 0}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('admin.appointments.revenue')}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <Select
                value={status || 'all'}
                onValueChange={(value) => {
                  setStatus(value === 'all' ? undefined : value as AppointmentStatus);
                  setPage(1);
                }}
              >
        ...
        <CardContent className="p-6">
          ...
          {appointments.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-muted-foreground">{t('admin.appointments.noAppointments')}</p>
            </div>
          ) : (
            <>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('admin.appointments.bookingNumber')}</TableHead>
                    ...
                    <TableRow key={appointment.id}>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {appointment.clinic?.doctorProfile?.specialty &&
                              getLocalizedText(appointment.clinic.doctorProfile.specialty.name, locale)}
                          </p>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(value) =>
                            handleUpdateStatus(appointment, value as AppointmentStatus)
                          }
                      ...
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(value) =>
                            handleUpdatePaymentStatus(appointment, value as PaymentStatus)
                          }
                        >
          ...
          {selectedAppointment && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                <div>
                  <p className="text-sm text-muted-foreground">{t('admin.appointments.date')}</p>
                  <p className="font-medium">{formatDate(selectedAppointment.appointmentDate)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">{t('admin.appointments.service')}</p>
                  <p className="font-medium">
                    {getLocalizedText(selectedAppointment.serviceName, locale)}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">{t('admin.appointments.price')}</p>
                  <p className="font-medium">{formatCurrency(selectedAppointment.price)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">{t('admin.appointments.queue')}</p>
                  <p className="font-medium">{selectedAppointment.queueNumber || '-'}</p>
                </div>
              </div>

              <div className="flex gap-4 pt-4 border-t">
                <div className="flex-1">
                  <p className="text-sm text-muted-foreground mb-1">{t('admin.appointments.status')}</p>
                  {getStatusBadge(selectedAppointment.status)}
                </div>
                <div className="flex-1">
                  <p className="text-sm text-muted-foreground mb-1">{t('payment.status')}</p>
              ...
              {selectedAppointment.cancellationReason && (
                <div className="pt-4 border-t">
                  ...
                  <p className="text-error-600">{selectedAppointment.cancellationReason}</p>
                </div>
              )}
            </div>
          )}
      ...
      <Dialog open={isCancelDialogOpen} onOpenChange={setIsCancelDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('admin.appointments.cancelAppointment')}</DialogTitle>
            <DialogDescription>
