...
export function AdminSecretariesList() {
  ...
  const {
    data: secretariesResponse,
    isLoading,
    isError,
  } = useAdminSecretaries(filters);
  ...
  const [selectedAssignment, setSelectedAssignment] =
    useState<SecretaryAssignment | null>(null);
    ...
    <>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between">
            <div className="flex gap-2 flex-wrap flex-1">
              ...
              <Select
                value={clinicId || "all"}
                onValueChange={(value) => {
                  setClinicId(value === "all" ? undefined : value);
                  setPage(1);
                }}
              >
                ...
                <SelectContent>
                  <SelectItem value="all">
                    {t("admin.secretaries.allClinics")}
                  </SelectItem>
                  {clinics.map((clinic) => (
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="true">
                    {t("admin.secretaries.activeOnly")}
                  </SelectItem>
                  <SelectItem value="false">
                    {t("admin.secretaries.inactiveOnly")}
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          {secretaries.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-muted-foreground">
                {t("admin.secretaries.noSecretaries")}
              </p>
            </div>
          ) : (
            <>
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{selectedSecretary?.fullName}</DialogDescription>
          </DialogHeader>
          ...
          {selectedSecretary && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("admin.secretaries.status")}
                  </p>
                  <Badge
                    variant={
                      selectedSecretary.isActive ? "success" : "secondary"
                    }
                  >
              ...
              {selectedSecretary.assignments &&
                selectedSecretary.assignments.length > 0 && (
                  <div className="pt-4 border-t">
                    <h4 className="font-semibold mb-3 flex items-center gap-2">
                      <Building className="h-4 w-4" />
                      {t("admin.secretaries.assignedClinics")} (
                      {selectedSecretary.assignments.length})
                    </h4>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t("admin.secretaries.selectClinic")}
                  />
                </SelectTrigger>
                <SelectContent>
                  {clinics.map((clinic) => (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("admin.secretaries.removeTitle")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("admin.secretaries.removeConfirm", {
                clinic: selectedAssignment
                  ? getLocalizedText(selectedAssignment.clinic.name, locale)
                  : "",
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
