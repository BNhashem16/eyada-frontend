...
export function DoctorReport({ doctorProfileId }: DoctorReportProps) {
  ...
  const { data: report, isLoading, isError } = useDoctorDetailedReport(doctorProfileId, filters);
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{formatCurrency(report.totalCommission)}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{formatCurrency(report.totalPaid)}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">{t('admin.collections.balance')}</p>
                <p className={`text-2xl font-bold ${report.balance > 0 ? 'text-warning-600' : 'text-green-600'}`}>
            ...
            <CardContent className="p-6">
              {report.appointments.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-muted-foreground">{t('admin.collections.noAppointments')}</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t('admin.collections.bookingNumber')}</TableHead>
            ...
            <CardContent className="p-6">
              {report.payments.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-muted-foreground">{t('admin.collections.noPayments')}</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>{t('admin.collections.paymentMethod')}</TableHead>
                      <TableHead>{t('admin.collections.referenceNumber')}</TableHead>
                      <TableHead>{t('admin.collections.notes')}</TableHead>
                      <TableHead>{t('admin.collections.paymentDate')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {report.payments.map((payment) => (
                      <TableRow key={payment.id}>
                        <TableCell>
                          <Badge variant="success" className="font-bold text-base">
                            {formatCurrency(payment.amount)}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          {payment.paymentMethod
                            ? t(`admin.collections.${payment.paymentMethod.toLowerCase()}`)
                            : '-'}
                        </TableCell>
                        <TableCell className="font-mono text-sm">
      ...
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('admin.collections.recordPaymentDesc', { doctor: report.doctorName })}
            </DialogDescription>
          </DialogHeader>
