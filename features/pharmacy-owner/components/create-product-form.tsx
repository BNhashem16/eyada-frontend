...
import { useCreateProduct, useImageUpload } from "../hooks";
...
export function CreateProductForm() {
  ...
  // Cascading categories: root categories have no parentId
  ...
  const handleParentCategoryChange = (value: string) => {
    ...
    // If parent has no children, use parent as the categoryId
    const parent = rootCategories.find((cat) => cat.id === value);
    if (!parent?.children?.length) {
      setValue("categoryId", value, { shouldDirty: true });
    } else {
      // Clear categoryId until a child is selected
