...
export function useSecretaryClinicSchedules(clinicId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      return apiGet<ClinicSchedule[]>(SECRETARY_ENDPOINTS.CLINIC_SCHEDULES(clinicId));
    },
    enabled: !!clinicId,
    staleTime: 1000 * 60 * 5,
  });
...
export function useSecretaryCreateSchedule() {
  ...
  return useMutation({
    mutationFn: async ({ clinicId, data }: { clinicId: string; data: Partial<ClinicSchedule> }) => {
      return apiPost<ClinicSchedule>(SECRETARY_ENDPOINTS.CLINIC_SCHEDULES(clinicId), data);
    },
    onSuccess: (_, { clinicId }) => {
      queryClient.invalidateQueries({ queryKey: ['secretary-clinic-schedules', clinicId] });
    },
  });
}
...
export function useSecretaryDeleteSchedule() {
  ...
  return useMutation({
    mutationFn: async ({ clinicId, scheduleId }: { clinicId: string; scheduleId: string }) => {
      return apiDelete(SECRETARY_ENDPOINTS.CLINIC_SCHEDULE(clinicId, scheduleId));
    },
    onSuccess: (_, { clinicId }) => {
      queryClient.invalidateQueries({ queryKey: ['secretary-clinic-schedules', clinicId] });
    },
