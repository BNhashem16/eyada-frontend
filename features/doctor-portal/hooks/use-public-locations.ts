...
export function usePublicStates(filters: PublicStatesFilters = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return apiGet<PaginatedResponse<State>>(`${PUBLIC_ENDPOINTS.STATES}?${params.toString()}`);
    },
    staleTime: 1000 * 60 * 5,
  });
}
...
export function usePublicCities(filters: PublicCitiesFilters = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return apiGet<PaginatedResponse<City>>(`${PUBLIC_ENDPOINTS.CITIES}?${params.toString()}`);
    },
    enabled: !!stateId,
    staleTime: 1000 * 60 * 5,
  });
