...
export function useDoctorSecretaries() {
  return useQuery({
    ...
    queryFn: async () => {
      return apiGet<SecretaryWithClinics[]>(
        DOCTOR_SECRETARIES_ENDPOINTS.SECRETARIES,
      );
    },
...
export function useClinicSecretaries(clinicId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      return apiGet<SecretaryAssignment[]>(
        DOCTOR_SECRETARIES_ENDPOINTS.CLINIC_SECRETARIES(clinicId),
      );
    },
    enabled: !!clinicId,
    staleTime: 1000 * 60 * 5,
...
export function useAssignSecretary() {
  ...
  return useMutation({
    mutationFn: async (data: AssignSecretaryData) => {
      return apiPost<SecretaryAssignment>(
        DOCTOR_SECRETARIES_ENDPOINTS.ASSIGN,
        data,
      );
    },
    onSuccess: () => {
...
export function useUpdateSecretary() {
  ...
  return useMutation({
    mutationFn: async ({
      secretaryUserId,
      ...data
    }: UpdateSecretaryData & { secretaryUserId: string }) => {
      return apiPatch(
        DOCTOR_SECRETARIES_ENDPOINTS.SECRETARY(secretaryUserId),
        data,
      );
    },
    onSuccess: () => {
...
export function useUpdateSecretaryAssignment() {
  ...
  return useMutation({
    mutationFn: async ({
      assignmentId,
      ...data
    }: UpdateAssignmentData & { assignmentId: string }) => {
      return apiPatch<SecretaryAssignment>(
        DOCTOR_SECRETARIES_ENDPOINTS.ASSIGNMENT(assignmentId),
        data,
      );
    },
    onSuccess: () => {
