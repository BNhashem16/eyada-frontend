...
export function DoctorAppointmentDetails({ appointmentId }: AppointmentDetailsProps) {
  ...
  const { data: appointment, isLoading, isError } = useDoctorAppointment(appointmentId);
    ...
    switch (appointment.status) {
      ...
      case AppointmentStatus.CONFIRMED:
        return [
          ...
          { status: AppointmentStatus.NO_SHOW, label: t('appointments.actionNoShow'), icon: XCircle, variant: 'secondary' as const },
        ];
      case AppointmentStatus.CHECKED_IN:
        return [
          { status: AppointmentStatus.COMPLETED, label: t('appointments.actionComplete'), icon: CheckCircle, variant: 'success' as const },
        ];
      default:
        return [];
    }
      ...
      {canEditMedicalNotes && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={handleSaveMedicalNotes} disabled={updateNotesMutation.isPending}>
